...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        const CAmount collateral = coins.vout[vin.prevout.n].nValue/COIN;
        if(!isValidMasternode(collateral, height)) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %"PRId64" PHASE, masternode=%s\n", collateral, vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when MN_COLLATERAL PHASE tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for MN_COLLATERAL PHASE tx -> 1 confirmation
