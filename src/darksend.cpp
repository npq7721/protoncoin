...
{
    if(fLiteMode) return; // ignore all Phase related functionality
...
/*
    ...
    allow endless transaction that would bloat Phase and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(isValidMasternode(out.nValue/COIN, height) && out.scriptPubKey == payee) return true;
    }
...
{
    if(fLiteMode) return; // disable all Phase specific functionality
